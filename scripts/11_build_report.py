...
def _engine_extras(daily: pd.DataFrame) -> dict:
    ...
    if "gross_exposure" in daily.columns:
        out["avg_gross_exposure"] = float(pd.to_numeric(daily["gross_exposure"], errors="coerce").mean())
...
def write_markdown_report(
    ...
    lines.append("Autogeneriert aus den aktuellen Pipeline-Artefakten.\n")
    ...
    if eq_vt_png.exists():
        lines.append(f"### Equity Curve (Engine, vol target 10%)\n\n![](figures/{eq_vt_png.name})\n")
    ...
    lines.append("## Notes\n")
    lines.append("- Sensitivitätsgrids dienen der Robustheitsprüfung, nicht der Parameteroptimierung.\n")
...
def main() -> None:
    ...
    plot_equity(reports / "backtest_engine_daily.csv", figs / "equity_engine.png", "Equity (Backtest Engine)")
    plot_equity(
        reports / "backtest_engine_daily_vt10.csv",
