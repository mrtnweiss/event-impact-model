...
def _key_artifacts_block() -> list[str]:
    lines: list[str] = []
    lines.append("## Key artifacts\n")
    lines.append("- `reports/event_study_summary.csv`\n")
    lines.append("- `reports/event_study_robustness.csv`\n")
    lines.append("- `data/processed/cv_metrics.csv`, `data/processed/cv_metrics_lgbm.csv`\n")
    lines.append(
...
def _reproduce_block() -> list[str]:
    lines: list[str] = []
    lines.append("## Reproduce (local)\n")
    lines.append("Install:\n")
    lines.append('```bash\npython -m pip install -U pip\npython -m pip install -e ".[dev]"\n```\n')
    lines.append("Run (end-to-end, requires network):\n")
    lines.append(
    ...
    lines.append("Offline smoke (fixtures only):\n")
    lines.append(
...
def write_markdown_report(
    ...
    lines.extend(_method_block())
    lines.extend(_key_artifacts_block())
    lines.extend(_reproduce_block())
