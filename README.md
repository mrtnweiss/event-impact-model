...
- Runs a costed backtest and a daily portfolio engine (delay, horizon, limits, optional vol targeting)
- Runs robustness sanity checks (shuffles / subsamples)
...
scripts/                  # pipeline entrypoints (01..14 + run_all)
...
- price start date and benchmark
...
### v0.1.0
- End-to-end pipeline from raw downloads to an out-of-sample, costed signal
- Correct event timestamping and NYSE trade-date mapping (incl. DST/holiday handling)
- Event study with pre-trend checks and BH-FDR subgroup testing
- Baselines + LightGBM with walk-forward CV guardrails
- Costed backtests + robustness sanity checks
- Autogenerated report and reproducible artifacts
